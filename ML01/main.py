...
no_dup = df.drop_duplicates(['Name', 'Author'])  # more robust
...
best_nf_authors = df.groupby(['Author', 'Genre']).agg({'Name': 'count'}).unstack()['Name', 'Non Fiction'].sort_values(
    ascending=False)[:11]
best_f_authors = df.groupby(['Author', 'Genre']).agg({'Name': 'count'}).unstack()['Name', 'Fiction'].sort_values(
    ascending=False)[:11]
...
with plt.style.context('Solarize_Light2'):
    ...
    ax[1].set_yticks(np.arange(len(best_f_authors)))
...
ax[0].barh(y=np.arange(len(top_authors)), width=top_authors.values)
ax[0].set_yticks(np.arange(len(top_authors)))  # Set tick positions
ax[0].set_yticklabels(top_authors.index, fontweight='semibold')  # Now safe
